...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    public static void main(String[] a) {
        try {
            // โหลดรูปจากไฟล์
            File f = new File("src/P.png");
            System.out.println("ABS PATH = " + f.getAbsolutePath());
            System.out.println("EXISTS = " + f.exists());
            BufferedImage img = ImageIO.read(new File("src/P.png"));
            int w = img.getWidth();
            int h = img.getHeight();

            // สร้างเต่า
            Turtle santa = new Turtle();
            Turtle.bgcolor("white");

            santa.speed(0);          // เร็วสุด
            santa.penColor("black");
            santa.width(1);
            santa.up();
            ...
            double canvasH = 1600;
            int stepx = 8;
            int stepy = 4;// ยิ่งน้อย ยิ่งละเอียด
            int pixelSize = (8);
            santa.shape("square");
            santa.shapeSize(pixelSize, pixelSize);
            ...
            for (int y = 0; y < h; y += stepy) {
                for (int x = 0; x < w; x += stepx) {
                    ...
                    // แปลงพิกเซล → พิกัดเต่า
                    double cx = (x / (double) w - 0) * canvasW;
                    double cy = ((h - y) / (double) h - 0) * canvasH;

                    // ตั้งสีของพิกเซล
                    santa.fillColor(c);
                    santa.outlineColor(c);

                    // วางสี่เหลี่ยม 1 เม็ด
                    santa.up();
                    santa.setPosition(cx, cy);
                    santa.stamp();
                }
            }
